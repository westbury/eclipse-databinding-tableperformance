 ...
 *
 */
public class JfaceTableWithGlobalListener {

	public JfaceTableWithGlobalListener(Shell shell) {
		...
		for( int i = 0; i < 200000; i++ ) {
			ModelObject element = new ModelObject();
		...
		ViewerSupport.bind(viewer, elements, 
			    new IValueProperty[] { 
				ModelObject.prop1Property,
				ModelObject.prop2Property,
				ModelObject.prop3Property,
				}
			    );
			...
			public void run() {
				try {
					...
					current.asyncExec(new Runnable() {
						@Override
						public void run() {
							((ModelObject)elements.get(2)).setProp1("update 1");
						}
					});
					Thread.sleep(5000);
					current.asyncExec(new Runnable() {
						@Override
						public void run() {
							((ModelObject)elements.get(4)).setProp2("update 2");
						}
					});
					Thread.sleep(5000);
					current.asyncExec(new Runnable() {
					...
					current.asyncExec(new Runnable() {
						@Override
						public void run() {
							((ModelObject)elements.get(12)).setProp3("update 3");
						}
					});
	...
	public static void main(String[] args) {
		...
		Realm.runWithDefault(SWTObservables.getRealm(display), new Runnable() { 
			public void run() { 
				...
				new JfaceTableWithGlobalListener(shell);
				shell.open ();
